...
bool UI_Create(){
   ...
   ObjectSetInteger(0,UI_BG,OBJPROP_XSIZE,430);
   ObjectSetInteger(0,UI_BG,OBJPROP_YSIZE,275);
   ...
   ObjectSetString(0,UI_HEADER,OBJPROP_TEXT,"SOFTI BLOOMBERG RADAR");
   ...
   ObjectSetInteger(0,UI_BTN,OBJPROP_XDISTANCE,300);
   ObjectSetInteger(0,UI_BTN,OBJPROP_YDISTANCE,24);
   ObjectSetInteger(0,UI_BTN,OBJPROP_XSIZE,120);
   ObjectSetInteger(0,UI_BTN,OBJPROP_YSIZE,24);
...
void UI_Update(){
   ...
   txt += "SESSION/RR: " + ((InpUseSessionQuality||InpUseRRWeighting)?"ON":"OFF") + "\n";
   txt += "\n";
   ...
   // SEND RADAR UPDATE TO FLASH
   string radarJson = "{\"event\": \"radar_update\", \"mode\": \""+mode+"\", \"opportunities\": \""+line+"\"}";
   SendToFlash(radarJson);
}
...
int OnInit(){
   ...
   UI_Update();
   LogPrint("SOFTI AI ANALYZER - ESECUTOR STARTED");
